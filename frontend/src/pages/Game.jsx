...
export default function Game() {
  ...
  async function submitTurn(dartOverride) {
    ...
    try {
      ...
      if (result.legWon) {
        // Snapshot scores BEFORE fetching updated game (which resets legs_won)
        const legSnapshot = game.mode === 'singles'
          ? game.players.map(p => ({ id: p.id, name: p.name, legs_won: p.legs_won }))
          : game.teams.map(t => ({ id: t.id, name: t.name, legs_won: t.legs_won }));
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px", textAlign: "center", gap: "20px", background: "var(--bg)" }}>
        ...
        <div className="card" style={{ width: "100%", maxWidth: "360px", padding: "16px" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
              {legResult.legSnapshot?.map((entry) => (
                <div key={entry.id} style={{ textAlign: "center" }}>
                  <p style={{ fontSize: "12px", color: "var(--muted)", marginBottom: "2px" }}>{entry.name}</p>
                  <p style={{ fontSize: "28px", fontFamily: "Bebas Neue", color: "var(--text)" }}>{entry.legs_won}</p>
                </div>
              ))}
            </div>
          </div>
