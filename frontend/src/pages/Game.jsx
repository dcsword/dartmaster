...
export default function Game() {
  ...
  useEffect(() => {
    ...
    if (score <= 170 && score > 1) {
      api.getCheckout(id, score, game.ruleset)
  ...
  function addDart(scoreVal, mult, isBull = false) {
    ...
    if (isBull) {
      ...
      if (score > remaining) { setError(`Score ${score} exceeds remaining ${remaining}`); setTimeout(() => setError(""), 2000); return; }
      setDarts((prev) => [...prev, { display, value: display, score, baseValue: scoreVal, multiplier: 1, isBull: true }]);
      setInputVal(""); setError(""); return;
    }
  ...
  function undoDart() { setDarts((prev) => prev.slice(0, -1)); setInputVal(""); }
    ...
    try {
      ...
      const result = await api.submitTurn(id, {
        darts: dartsToSubmit.map((d) => {
          if (d.isBull) return d.value;
          if (d.multiplier === 2) return d.value;
          if (d.multiplier === 3) return d.value;
          return d.score;
        }),
        playerId: player?.id,
        teamId: team?.id,
      ...
      if (result.legWon) {
        ...
        setGame(updated); setDarts([]); setInputVal(""); setMultiplier(1);
  ...
  async function handleBust() {
    ...
    try {
      ...
      const updated = await api.getGame(id);
      setGame(updated); setDarts([]); setInputVal("");
      advanceTurn();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  if (legResult) {
    ...
    function legsWonInSet(entityId, mode) {
      return legsInCurrentSet.filter((l) =>
        mode === "singles" ? l.winner_id === entityId : l.winner_team_id === entityId
      ).length;
    }
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px", textAlign: "center", gap: "20px", background: "var(--bg)" }}>
        ...
        <div>
          ...
          <p style={{ color: "var(--text)", fontSize: "18px", fontWeight: 500 }}>{legResult.setWon ? "wins the set!" : "wins the leg!"}</p>
        </div>
        ...
        <div className="card" style={{ width: "100%", maxWidth: "360px", padding: "16px" }}>
          {multiSet && (
            <div style={{ marginBottom: "12px", paddingBottom: "12px", borderBottom: "1px solid var(--border)" }}>
              ...
              <div style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
                {g.mode === "singles"
                  ? g.players?.map((p) => (<div key={p.id} style={{ textAlign: "center" }}><p style={{ fontSize: "12px", color: "var(--muted)", marginBottom: "2px" }}>{p.name}</p><p style={{ fontSize: "28px", fontFamily: "Bebas Neue", color: "var(--text)" }}>{p.sets_won}</p></div>))
                  : g.teams?.map((t) => (<div key={t.id} style={{ textAlign: "center" }}><p style={{ fontSize: "12px", color: "var(--muted)", marginBottom: "2px" }}>{t.name}</p><p style={{ fontSize: "28px", fontFamily: "Bebas Neue", color: "var(--text)" }}>{t.sets_won}</p></div>))
                }
              </div>
            </div>
          )}
          <div>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
              {g.mode === "singles"
                ? g.players?.map((p) => (<div key={p.id} style={{ textAlign: "center" }}><p style={{ fontSize: "12px", color: "var(--muted)", marginBottom: "2px" }}>{p.name}</p><p style={{ fontSize: "28px", fontFamily: "Bebas Neue", color: "var(--text)" }}>{legsWonInSet(p.id, "singles")}</p></div>))
                : g.teams?.map((t) => (<div key={t.id} style={{ textAlign: "center" }}><p style={{ fontSize: "12px", color: "var(--muted)", marginBottom: "2px" }}>{t.name}</p><p style={{ fontSize: "28px", fontFamily: "Bebas Neue", color: "var(--text)" }}>{legsWonInSet(t.id, "teams")}</p></div>))
              }
            </div>
          </div>
        </div>
