...
export default function GameDetail() {
  ...
  function dartValue(d) {
    ...
    if (d.multiplier === 3) return `T${d.score / 3}`;
    return `${d.score}`;
  }
  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', padding: '20px 16px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ fontSize: '40px', fontWeight: 800, color: 'var(--text)' }}>GAME DETAIL</h1>
                  ...
                  <div key={leg.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      {(leg.winner_name || leg.winner_team_name) && (
                        <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>🏆 {leg.winner_team_name || leg.winner_name}</span>
                      )}
